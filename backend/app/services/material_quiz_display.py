...
def detect_link_wrapper(title: str, file_type: str) -> bool:
    """True for Moodle URL/link rows that duplicate a real lecture/lab file."""
    t = (title or "").strip()
    ft = (file_type or "").lower().strip()
    if _LECTURE_LINK_TITLE_RE.search(t) or _LAB_LINK_TITLE_RE.search(t):
        return True
    if ft in _LINK_FILE_TYPES and re.search(r"(?i)\b(?:lecture|lec|lab)\s*#?\d+", t):
        return True
    return False
...
def extract_material_number(title: str, material_kind: str) -> int:
    """Number immediately after lecture/lec/lab only — never SWE423/CSC344."""
    t = title or ""
...
def material_sort_group(title: str, is_non_quiz: bool) -> int:
    """Legacy helper — prefer material_kind + _KIND_SORT_GROUP."""
...
def material_sort_number(title: str, sort_group: Optional[int] = None) -> int:
    """Legacy helper — prefer extract_material_number with material_kind."""
    t = title or ""
...
def material_display_sort_key(display: Dict[str, Any]) -> Tuple[int, int, int, str]:
    """kind priority → material_number → real file before link → title."""
    return (
        int(display.get("sort_group") or 5),
        int(display.get("material_number") or 9999),
        int(display.get("sort_link_rank") or 0),
...
def apply_course_material_visibility(displays: List[Dict[str, Any]]) -> None:
    """
    After per-material resolve, detect URL/link duplicates of real lecture/lab files
    and move them to Other Moodle items.
    """
    real_keys: Set[Tuple[str, int]] = set()
    for d in displays:
        if d.get("is_link_wrapper"):
            continue
        ...
        if num is None or num >= 9999:
            continue
        if kind == "lecture":
            real_keys.add(("lecture", num))
        elif kind == "lab":
            real_keys.add(("lab", num))
    ...
    for d in displays:
        ...
        if sibling:
            ...
            # Keep lecture/lab number for debug sort; only visibility changes.
            d["sort_group"] = 5
...
def resolve_material_display(doc: Dict[str, Any]) -> Dict[str, Any]:
    ...
    if is_link_wrapper:
        is_educ = bool(
            _LECTURE_TYPE_RE.search(title or "")
            or _LAB_TYPE_RE.search(title or "")
        )
    else:
    ...
    base_fields = {
        ...
        "sort_group_label": _SORT_GROUP_LABELS[sort_group] if sort_group < len(_SORT_GROUP_LABELS) else "Other",
